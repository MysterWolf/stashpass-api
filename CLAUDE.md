...
- [x] `DELETE /operators/:id/locations/:locationId` — soft-delete (`active = FALSE`); CIRCLES_API_SECRET auth
- [x] `GET /operators/:id/profile` — now includes `locations` array in response (parallel fetch)
- [x] Types: `OperatorLocation` interface in types.ts; `LocationData` + service functions in operator.service.ts
...
| 2026-06-16 | Profile upsert — POST and PUT /operators/:id/profile both upsert now; patchProfile falls through to setProfile when no profile exists; admin dashboard never needs to track create vs update state |
| 2026-06-22 | Dominance — 008_strain_dominance.sql adds `dominance VARCHAR(20)` with CHECK constraint to strains table. StrainBody Zod schema extended with `dominance` enum (nullable, optional). Migration applied to Railway prod. |
| 2026-06-22 | Strain discovery queue — 009_strain_queue.sql (strain_queue table: name, type, device_id, device_ids JSONB, surface_count, status CHECK, strain_id FK). routes/queue.ts: POST /queue/strains (public, exists/queued response, surface_count increment, device_ids dedup), GET /queue/strains (admin, sorted by surface_count DESC), PATCH /queue/strains/:id (admin status + strain_id). GET /strains accepts device_id param (no-op until Prompt 2 adds per-device scoping). |
